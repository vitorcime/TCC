...
print("trocou")
modelo = load_model("./modelosEnergiaSimples/modelo" + sys.argv[1] + ' ' + str(sys.argv[2]))
...
result = (classification_report(y_pred = resultado, y_true = identificacoes_teste, output_dict= True))
print(result['accuracy'])
txt = open('runModel.txt', 'r')
conteudo = txt.readlines()
conteudo.append('Tipo limiar: ' + str(sys.argv[1]) + ' Acuracia: ' + str(result['accuracy']) + ' patch: ' + str(sys.argv[2]))
conteudo.append('\n')
txt = open('runModel.txt', 'w')
